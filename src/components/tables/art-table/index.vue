...
<script setup lang="ts">
  ...
  const { width: tableContainerWidth } = useElementSize(tableContainerRef)
  ...
  const paginationLayout = computed(() => {
    if (tableContainerWidth.value < 768) {
      return LAYOUT.MOBILE
    } else if (tableContainerWidth.value < 1024) {
      return LAYOUT.IPAD
    } else {
  ...
  const defaultPaginationOptions = computed<PaginationOptions>(() => ({
    ...
    size: 'default',
    pagerCount: tableContainerWidth.value > 1200 ? 7 : 5
  }))
  ...
  const findTableHeader = () => {
    ...
    const tableHeader = props.headerElement
    if (tableHeader) {
