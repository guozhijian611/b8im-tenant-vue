    ...
    >
      <ElRow :gutter="gutter">
        ...
        <ElCol
          :xs="24"
          :sm="24"
          :md="getDefaultColSpan('md')"
          :lg="getDefaultColSpan('lg')"
          :xl="getDefaultColSpan('xl')"
          class="action-column"
        >
          <div class="action-buttons-wrapper" :class="actionButtonsClass">
...
<script setup lang="ts">
  ...
  import {
    ...
    resolveResponsiveSpanValue,
    type ResponsiveSpanValue
  ...
  const getDefaultColSpan = (breakpoint: ResponsiveBreakpoint): number => {
    return calculateResponsiveSpan(undefined, span.value, breakpoint)
  }

  const collapseBaseSpan = computed(() => {
    if (typeof props.span === 'number') {
      return props.span
    }

    return resolveResponsiveSpanValue(props.span, 'lg') ?? 6
  })
  ...
  const visibleFormItems = computed(() => {
    ...
    if (shouldShowLess) {
      const maxItemsPerRow = Math.floor(24 / collapseBaseSpan.value) - 1
  ...
  const shouldShowExpandToggle = computed(() => {
    ...
    return (
      !props.isExpand &&
      props.showExpand &&
      filteredItems.length > Math.floor(24 / collapseBaseSpan.value) - 1
    )
  })
   ...
   * 操作按钮样式
   */
  const shouldAlignActionsLeft = computed(
    () => props.items.filter((item) => !item.hidden).length <= props.buttonLeftLimit
  )

  const actionButtonsClass = computed(() =>
    shouldAlignActionsLeft.value ? 'action-buttons--start' : 'action-buttons--end'
  )
...
<style lang="scss" scoped>
  .art-search-bar {
    ...
    .action-column {
      ...
      .action-buttons-wrapper.action-buttons--start {
        justify-content: flex-end;
      }
  ...
  @media (width > 500px) {
    .art-search-bar {
      .action-column {
        ...
        .action-buttons-wrapper.action-buttons--end {
          justify-content: flex-end;
        }
      }
    }
