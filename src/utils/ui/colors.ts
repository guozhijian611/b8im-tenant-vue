...
/**
 * 生成暗色模式下更易读的主题强调色
 * 保留原主题色作为 Element Plus 主色，仅提升文本/图标等强调色的可读性
 * @param color 主题颜色
 * @param isDark 是否为暗色模式
 * @returns 可读的主题强调色
 */
function getReadableThemeColor(color: string, isDark: boolean): string {
  if (!isDark) {
    return color
  }

  const minimumReadableLuminance = 0.2
  if (getRelativeLuminance(color) >= minimumReadableLuminance) {
    return color
  }

  const liftLevels = [0.12, 0.2, 0.28, 0.36, 0.44, 0.52, 0.6]
  for (const level of liftLevels) {
    const candidateColor = getLightColor(color, level)
    if (getRelativeLuminance(candidateColor) >= minimumReadableLuminance) {
      return candidateColor
    }
  }

  return getLightColor(color, 0.6)
}
...
export function setElementThemeColor(color: string): void {
  ...
  const isDark = useSettingStore().isDark
  const readableThemeColor = getReadableThemeColor(color, isDark)
  const readableThemeHoverColor = getLightColor(readableThemeColor, 0.12)
  const readableThemeActiveColor = getDarkColor(readableThemeColor, 0.12)
  const readableThemeDisabledColor = getLightColor(readableThemeColor, 0.24)

  elStyle.setProperty('--el-color-primary', color)
  elStyle.setProperty('--theme-color', readableThemeColor)
  elStyle.setProperty('--theme-color-hover', readableThemeHoverColor)
  elStyle.setProperty('--theme-color-active', readableThemeActiveColor)
  elStyle.setProperty('--theme-color-disabled', readableThemeDisabledColor)
  elStyle.setProperty('--theme-color-plain-bg', hexToRgba(readableThemeColor, 0.16).rgba)
  elStyle.setProperty('--theme-color-plain-border', hexToRgba(readableThemeColor, 0.38).rgba)
